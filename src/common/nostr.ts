...
export const fetchFormResponses = async (
  ...
  const uniqueCandidatePubkeys = Array.from(
    new Set(candidatePubkeys.filter(Boolean)),
  );
  const candidateRelayMap = await fetchRelayLists(uniqueCandidatePubkeys);
  const candidateRelays = Array.from(candidateRelayMap.values()).flat();
  const relays = getDiscoveryRelays([...extraRelays, ...candidateRelays]);
  const events = await nostrRuntime.querySync(relays, {
    kinds: [EventKinds.FormResponse],
    "#a": [formCoordinate],
    ...(uniqueCandidatePubkeys.length > 0
      ? { authors: uniqueCandidatePubkeys }
      : {}),
  });
